	...
	if s.perspective == protocol.PerspectiveClient {
		...
		if s.retrySrcConnID != nil { // a Retry was performed
			if params.RetrySourceConnectionID == nil {
				return qerr.NewError(qerr.TransportParameterError, fmt.Sprintf("missing retry_source_connection_id"))
			}
			...
			return qerr.NewError(qerr.TransportParameterError, fmt.Sprintf("received retry_source_connection_id, although no Retry was performed"))
		}
	}
